...
export class HealthSystem extends System {
  update(ecs: ECS, deltaTime: number): void {
    ...
    for (const entity of entities) {
      ...
      // 3. If health is below or equal to 0, log that the entity is dead
      if (health.current <= 0) {
        logger("warn", `Entity ${entity} died!`);
      }

      // Example logic: Regenerate health over time
      if (health.current < health.max) {
        health.current = Math.min(health.current + 1 * deltaTime, health.max);
        health.current = Math.round(health.current * 100) / 100; // Round to nearest integer
        logger("info", `Entity ${entity} health regenerated to ${health.current}/${health.max} HP.`);
      }
    }
  }
}